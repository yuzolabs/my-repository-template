...
Dev Container では、ワークツリーごとに一意のポートが自動的に割り当てられます。

### ポートの割り当てルール

- ベースポート: 31000
- メインリポジトリ: 31000
- ワークツリー: 31000 + ワークツリーインデックス

### ポートの確認方法

1. Dev Container を開く
2. `.devcontainer/.env` ファイルを確認
3. `DEV_PORT` の値が使用ポート

### 使用方法

コンテナ内でサーバーを起動すると、自動的にホスト側からアクセスできます。
...
ホスト側からは `http://localhost:<DEV_PORT>` でアクセスできます。

### 制限事項

- ポート競合が発生した場合は、手動で `.devcontainer/.env` の `DEV_PORT` を変更してください
- 最大1000ワークツリーまで対応（31000-31999）
